...
struct entities_renderer {
	void render(sf::RenderWindow& window, board_container& board, const std::unordered_map<std::uint64_t, std::int32_t>& healths) {

		board.for_each([this, &healths, &window](std::uint64_t entity_id, std::uint64_t col, std::uint64_t row) {
			if (entity_id != -1) {
