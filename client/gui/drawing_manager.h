...
struct drawing_manager : base_manager<drawable::ptr_type, drawable::ptr_type> {
    static drawable::ptr_type drawer_for(std::uint32_t entity_id) {
    ...
    static bitmap_key get_bitmap_key_for(std::uint32_t entity_id) {
        return map_.at(entity_id)->get_bitmap_key();
    }
};

template <>
inline void add_component_of_type<drawable::ptr_type>(std::uint32_t entity_id, const drawable::ptr_type& component) {
