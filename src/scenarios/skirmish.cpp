...
struct map_factory {
  ...
  static void initialize(const std::string& map_name) {
    static std::unordered_map<std::string, std::function<void()>> map_initializers = {
        map_initializer("winter land", &winter_land), map_initializer("battlefield", &battlefield)};
...
auto get_entities_starting_positions() {
  std::array<std::pair<std::uint32_t, std::uint32_t>, 8> positions = {
      pos(2, 2), pos(12, 2), pos(2, 4), pos(12, 4), pos(2, 6), pos(12, 6), pos(2, 8), pos(12, 8)};
  return std::move(positions);
}

std::vector<std::string> get_possible_to_choose_entities() {
  std::vector<std::string> result = {
      "wretch", "wizzard", "warrior", "troll",
      //"rocket_thrower",
      "eye", "spider", "soul_hunter", "sorcerer", "sniper", "shooter", "samurai_rat", "saberhand",
      //"robot",
      "poisoner", "native", "monk", "mechanical_saw",
      //"killer",
      //"handthrower",
      "guardian", "grenadier", "golem",
      //"gin",
      "reaper", "giant", "druid", "droid",
      //"dragon",
      "destroyer",
      //"creature",
      "commander"
...
std::string create_skirmish(const std::string& map_name,
  ...
  set_every_turn_callback(
      ...
      [=, entities = create_entities_container()](const turn_callback_info& info) mutable {
        ...
        sender::send(
            make_move_entity_message(entity_id, game_control().selected_index_, new_index));
        ...
        if (info.is_ending) {
          ...
          std::cout << "Need to destroy the following entities: " << entities_to_choose.size() << "\n";
