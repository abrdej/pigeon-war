...
{
	indexes.clear();
	game::get<board>().for_each([&player_id, &indexes](std::uint32_t entity_id, std::uint32_t col, std::uint32_t row) {
		if (entity_id != -1 && game::get<players_manager>().player_entity(player_id, entity_id))
			indexes.push_back(game::get<board>().to_index(col, row));
	});
}
...
{
	auto entity_id = game::get<board>().at(entity_index);
	if (entity_id != -1)
