...
//std::uint32_t players::active_player_ = 0;
//std::unordered_map<std::uint32_t, players::player_id_type> players::relation_map_;
...
//}
//
//void players::add_entity_for_player(const player_id_type& name, std::uint32_t entity_id)
//{
//	auto it = relation_map_.insert(std::make_pair(entity_id, name));
//	it.first->second = name;
//}
//
//void players::add_neutral_entity(std::uint32_t entity_id)
//{
//	auto it = relation_map_.insert(std::make_pair(entity_id, neutral_id));
//	it.first->second = neutral_id;
//}
//
//bool players::player_entity(player_id_type player_id, std::uint32_t entity_id)
//{
//	return relation_map_[entity_id] == player_id;
//}
//
//bool players::enemy_entity(player_id_type player_id, std::uint32_t entity_id)
//{
//	return relation_map_[entity_id] != player_id && relation_map_[entity_id] != neutral_id;
//}
//
//bool players::neutral_entity(std::uint32_t entity_id) {
...
//}
//
//std::uint32_t players::active_player_index() {
...
//}
//
//players::player_id_type players::player_for_entity(std::uint32_t entity_id) {
...
{
	indexes.clear();
	board::for_each([&player_id, &indexes](std::uint32_t entity_id, std::uint32_t col, std::uint32_t row) {
