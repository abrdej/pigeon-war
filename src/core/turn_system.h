...
public:
    ...
    turn_scoped_connection set_callback(const frequency_types& frequency,
        ...
        return turn_scoped_connection(
                next_turn_signal.connect_extended(turn_callback_wrapper(frequency, duration, callback)));
    }
...
protected:
    ...
    void on_every_turn(Callback callback) {
        on_every_turn_holder = std::move(
                game::get<turn_system>().set_callback(frequency_types::every_turn, 0, callback));
    }

    template <typename Callback>
    void on_every_two_turns_from_this(Callback callback) {
        on_every_two_turns_from_this_holder = std::move(
                game::get<turn_system>().set_callback(frequency_types::every_two_turns_from_this, 0, callback));
    }

    template <typename Callback>
    void after_player_turn(std::uint32_t entity_id, Callback callback) {
        after_player_turn_holder = std::move(
                game::get<turn_system>().set_callback(frequency_types::every_turn, 0, [entity_id, callback]() {
    ...
    void on_every_two_turns_from_next(Callback callback) {
        on_every_two_turns_from_next_holder = std::move(
                game::get<turn_system>().set_callback(frequency_types::every_two_turns_from_next, 0, callback));
    }
