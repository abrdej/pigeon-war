...
class turn_system {
  struct turn_callback_wrapper {
    ...
    turn_callback_wrapper(const frequency_types& frequency, int32_t duration,
                          std::function<void(turn_callback_info&)> callback)
        : frequency(frequency),
          duration(duration),
          number_of_calls(0),
 ...
 public:
  ...
  turn_scoped_connection set_callback(const frequency_types& frequency, int32_t duration,
                                      const extended_turn_callback& callback) {
    return turn_scoped_connection(
        next_turn_signal_.connect_extended(turn_callback_wrapper(frequency, duration, callback)));
  }

  turn_scoped_connection set_callback(const frequency_types& frequency, int32_t duration,
                                      const turn_callback& callback) {
    return turn_scoped_connection(next_turn_signal_.connect_extended(turn_callback_wrapper(
        frequency, duration, [callback](turn_callback_info&) { return callback(); })));
  }
...
inline auto make_every_two_turns_from_next_callback_holder(std::int32_t duration,
                                                           Callback callback) {
  return game::get<turn_system>().set_callback(frequency_types::every_two_turns_from_next, duration,
                                               callback);
}
...
inline void set_after_n_round_callback(std::int32_t duration, Callback callback) {
  game::get<turn_system>()
      .set_callback(frequency_types::after_n_rounds, duration, callback)
      .release();
}
...
class turn_callback_helper {
 protected:
  ...
  void on_every_turn(Callback callback) {
    on_every_turn_holder_ =
        std::move(game::get<turn_system>().set_callback(frequency_types::every_turn, 0, callback));
  }

  template <typename Callback>
  void on_every_two_turns_from_this(Callback callback) {
    on_every_two_turns_from_this_holder_ = std::move(game::get<turn_system>().set_callback(
        frequency_types::every_two_turns_from_this, 0, callback));
  }

  template <typename Callback>
  void after_player_turn(std::uint32_t entity_id, Callback callback) {
    after_player_turn_holder_ = std::move(game::get<turn_system>().set_callback(
        frequency_types::every_turn, 0, [entity_id, callback]() {
  ...
  void on_every_two_turns_from_next(Callback callback) {
    on_every_two_turns_from_next_holder_ = std::move(game::get<turn_system>().set_callback(
        frequency_types::every_two_turns_from_next, 0, callback));
  }
