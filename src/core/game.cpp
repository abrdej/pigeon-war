...
void game::on_board(std::uint32_t col, std::uint32_t row)
{
	...
	auto index = board_ref.to_index(col, row);

	if (board_ref.empty(index) && state_controller::actual_state_ == states_types::waiting)
		return;
	...
	else if (state_controller::actual_state_ == states_types::wait_for_action)
	{
		...
		else if (!board_ref.empty(index) && players_funcs::player_entity(index))
