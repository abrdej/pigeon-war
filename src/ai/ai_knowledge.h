...
struct ai_knowledge {
    ...
    knowledge_box& global() {
        return global_knowledge_box_;
    }

    knowledge_box& player(std::uint32_t player_id) {
        return players_knowledge_boxes_[player_id];
    }

    knowledge_box& entity(std::uint32_t entity_id) {
        return entities_knowledge_boxes_[entity_id];
    }
};
