...
#include <ai/movers.h>
#include <ai/nodes_compositions.h>
...
inline behavior_node_ptr make_ai<wolf>() {
    return ai::construct::make_sequence(
            std::make_shared<ai::nodes::find_lowest_health_target>(),
...
inline behavior_node_ptr make_ai<saurian>() {
    auto lowest_health_target_sequence = ai::construct::make_sequence(
            std::make_shared<ai::nodes::find_lowest_health_target>(),
            std::make_shared<ai::nodes::attack_enemy>());
...
inline behavior_node_ptr make_ai<golem>() {
    auto best_target_sequence = ai::construct::make_sequence(
            ...
            std::make_shared<ai::nodes::go_to>(),
            std::make_shared<ai::nodes::attack_enemy>());
...
inline behavior_node_ptr make_ai<pixie>() {
    return ai::construct::make_selector(
            std::make_shared<ai::nodes::run_around>(),
            std::make_shared<ai::nodes::attack_enemy>());
}
