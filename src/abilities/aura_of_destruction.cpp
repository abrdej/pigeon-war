...
aura_of_destruction::aura_of_destruction(std::uint32_t entity_id) : entity_id(entity_id) {
  on_receive_damage(
      entity_id,
      [this, entity_id](const damage_pack& dmg) mutable {
        ...
        if (value_for_power > 0) {
          ...
          if (enemy_id != no_damage_dealer) {
            damage_dealers::standard_damage_dealer(
                magic_damage(value_for_power, enemy_id, entity_id));
          }
        }
      },
