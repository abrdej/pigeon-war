...
void saw_passing::use(std::uint32_t index_on) {
	...
	game::get<board>().move(used_from_index, index_to_move);
	...
	if (xx != 0) {
		for (std::int32_t x = xx > 0 ? 1 : -1; std::abs(x) < std::abs(xx); x = xx > 0 ? x + 1 : x - 1) {
			...
			if (!game::get<board>().empty(index) && !game::get<players_manager>().enemy_entity(game::get<players_manager>().player_for_entity(entity_id), entity_id)) {
				damage_dealers::standard_damage_dealer(melee_damage(damage, game::get<board>().at(index), entity_id));
