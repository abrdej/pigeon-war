	...
	meteorite_holder = make_after_n_round_callback_holder(1,
														  [caster_id, index_on, damage]() mutable {
															  ...
															  if (!game::get<board>().empty(index_on)) {
																  damage_dealers::standard_damage_dealer(magic_damage(damage, game::get<board>().at(index_on), caster_id));
															  }

															  for (auto& index : neighbors) {
																  if (!game::get<board>().empty(index))
																	  damage_dealers::standard_damage_dealer(magic_damage(damage, game::get<board>().at(index), caster_id));
															  }
														  });
