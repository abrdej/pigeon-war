    ...
    set_damage_receiver(entity_id, [this, entity_id](health_field& health_pack, const damage_pack& dmg) mutable {
        ...
        for (auto&& neighbor : neighbors) {
            if (!game_board().empty(neighbor)) {
                if (players_funcs::player_entity(game_board().index_for(entity_id)) && players_funcs::enemy_entity(neighbor)) {
                    ++enemies_in_neighborhood;

                } else if (!players_funcs::player_entity(game_board().index_for(entity_id)) && players_funcs::player_entity(neighbor)) {
                    ++enemies_in_neighborhood;
                }
            }
        }
