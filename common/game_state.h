...
static const auto no_selection = std::numeric_limits<std::uint64_t>::max();

struct game_state {
	board_container board;
	std::unordered_map<std::uint64_t, std::int32_t> healths;
	std::unordered_map<std::uint64_t, bitmap_key> entities_bitmaps;
	std::unordered_map<std::uint64_t, std::vector<std::string>> entities_additional_effects;
	std::vector<animation_pack> animations_queue;
};

struct local_state {
	std::vector<std::uint64_t> possible_movements;
	std::unordered_set<std::uint64_t> valid_movements;
	std::uint64_t selected_index{no_selection};
