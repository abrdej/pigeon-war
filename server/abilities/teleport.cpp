...
void teleport::prepare(std::uint64_t for_index) {
	...
	states::state_controller::wait_for_action([this, for_index](std::uint64_t index)
											  {
												  return use(for_index, index);
											  });
}

void teleport::use(std::uint64_t from_index, std::uint64_t to_index) {
