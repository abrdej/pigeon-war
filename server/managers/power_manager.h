...
public:
    ...
    static inline void modify_power_by(sf::Uint64 entity_id, int value) {
        component_reference_for(entity_id).power += value;
    }
    static inline void set_power_to(sf::Uint64 entity_id, int value) {
        component_reference_for(entity_id).power = value;
    }
    static inline void set_power_to_base_power(sf::Uint64 entity_id) {
        auto& power_pack = component_reference_for(entity_id);
        power_pack.power = power_pack.base_power;
    }
    static inline int get_power_for(sf::Uint64 entity_id) {
        return component_for(entity_id).power;
    }
    static inline int get_base_power_for(sf::Uint64 entity_id) {
        return component_for(entity_id).base_power;
    }
};

template <>
inline void add_component_of_type<power_field>(sf::Uint64 entity_id, const power_field& component) {
